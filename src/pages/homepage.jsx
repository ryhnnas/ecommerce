...
const ProductCard = ({ product }) => {
    const cardStyles = { card: { border: '1px solid #e0e0e0', borderRadius: '12px', padding: '1rem', minWidth: '220px', fontFamily: "'Inter', sans-serif", backgroundColor: 'white', }, imageContainer: { position: 'relative', backgroundColor: '#f8f9fa', borderRadius: '8px', marginBottom: '1rem', aspectRatio: '1 / 1', }, wishlistIcon: { position: 'absolute', top: '10px', right: '10px', color: '#adb5bd', cursor: 'pointer', }, discountBadge: { position: 'absolute', top: '10px', left: '10px', backgroundColor: '#e0f3ff', color: '#007bff', borderRadius: '6px', padding: '2px 8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '12px', }, name: { fontWeight: '600', marginBottom: '0.5rem', color: '#212529', }, priceSection: { display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem', }, currentPrice: { fontWeight: 'bold', color: '#212529', }, originalPrice: { fontSize: '0.875rem', color: '#6c757d', textDecoration: 'line-through', }, discountInfo: { fontSize: '0.875rem', color: 'red', fontWeight: 'bold', }, footer: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', }, cartButton: { background: 'none', border: 'none', color: '#007bff', cursor: 'pointer', } };
    return (
        <div style={cardStyles.card}>
            ...
            {product.discount ? ( <> <div style={cardStyles.currentPrice}>{formatCurrency(product.discountedPrice)}</div> <div style={cardStyles.priceSection}> <div style={cardStyles.originalPrice}>{formatCurrency(product.originalPrice)}</div> <div style={cardStyles.discountInfo}>{product.discount}%</div> </div> </> ) : ( <div style={cardStyles.currentPrice}>{formatCurrency(product.price)}</div> )}
            <div style={cardStyles.footer}> <Rating count={product.rating} /> <button style={cardStyles.cartButton}><CartIcon/></button> </div>
        </div>
    );
};
